...
async def on_service_state_change_process(zc, service_type, name, cls):
    ...
    info_dict['properties'] = info_dict['_properties']#{k.decode():v.decode() for k,v in info.properties.items()}
...
class ZeroconfServiceTypes(object):
    ...
    def add_service(self, zc, service_type, name):
        ...
        asyncio.create_task(on_service_state_change_process(zc, service_type, name, self))
...
async def simple_find_service(loop, service=SERVICE, timeout=SERVICE_DISCOVERY_TIMEOUT, stop_after_first=STOP_AFTER_FIRST):
    ...
    result = await ZeroconfServiceTypes.find(zeroconf, service=service, timeout=timeout, stop_after_first=stop_after_first)
    #print(result)
...
if __name__ == '__main__':
    try:
        ...
        result = loop.run_until_complete(simple_find_service(loop, service=SERVICE, stop_after_first=True))
